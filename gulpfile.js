...
gulp.task('sassCompilation', ['normalize'], function () {
  return gulp.src('src/sass/**/*.+(scss|sass)')
    ...
    .pipe(autoprefixer([
      'last 7 versions', '> 1%', 'ie >= 9', 'and_chr >= 2.3' //, 'ie 8', 'ie 7'
    ], {
      cascade: true
    }))
...
gulp.task('mergeCssLibs', function () {
  return gulp.src([
      'src/css/temp/*.css', 'node_modules/select2/dist/css/select2.min.css', './src/includes/swiper.css', 'node_modules/animate.css/animate.min.css'
    ])
...
gulp.task('copyLibsScriptsToJs', ['copyJqueryToJs'], function () {
  return gulp.src([
      'node_modules/jquery-debouncedresize/index.js' // "умный" ресайз
      , 'node_modules/jquery-placeholder/jquery.placeholder.js' // поддержка плейсхолдера в старых браузерах
